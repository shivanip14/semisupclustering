...
def run_algo(algorithm, algorithm_name, X, y, n_fold, manually_annotate):
    ari_intermediate = 0
    ami_intermediate = 0
    ...
    print('{} avg over {} folds; SF = [{:.2f}], NF = [{:.2f}]: ARI = {}, AMI = {}'.format(algorithm_name, n_fold, algorithm.seed_fraction, algorithm.noise_fraction, ari, ami))
    return ari, ami
...
def run_KMeans(X, y, n_clusters, n_fold):
    ...
    ari = ari_intermediate/n_fold
    ami = ami_intermediate/n_fold
    print('K-Means avg over {} folds: ARI = {}, AMI = {}'.format(n_fold, ari, ami))
    return ari, ami